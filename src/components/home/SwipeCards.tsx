...
export default function SwipeCards({ projects }: SwipeCardsProps) {
  ...
  const startX = useRef(0);
  const startY = useRef(0); // ✅ NEW (vertical detect)
  const curX = useRef(0);
  ...
  function onPointerDown(e: React.PointerEvent) {
    ...
    dragging.current = true;
    startX.current = e.clientX;
    startY.current = e.clientY; // ✅ store Y
    curX.current = 0;
  ...
  function onPointerMove(e: React.PointerEvent) {
    ...
    // 🚫 Vertical scroll detect → ignore swipe
    if (Math.abs(dy) > Math.abs(dx)) return;

    curX.current = dx;

    const limitedDx = Math.min(dx, 15);
    ...
    if (frontEl) {
      gsap.set(frontEl, {
        x: limitedDx,
        rotation: getPos(0).rotation + limitedDx * 0.04,
      });
    }

    const progress = Math.max(0, Math.min(-limitedDx / 180, 1));
    ...
    <div className="flex flex-col items-center justify-center w-full py-10">
      <div
        ...
        style={{
          ...
          height: "520px",
          marginLeft: "-12px",
          touchAction: "pan-y", // ✅ IMPORTANT FIX
        }}
        onPointerDown={onPointerDown}
        onPointerMove={onPointerMove}
        onPointerUp={onPointerUp}
