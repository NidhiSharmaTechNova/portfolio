...
export default function StickyCards() {
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full h-[150vh] hidden min-[809px]:block">
        {projects.map((project, i) => (
          <div key={i} className="card absolute left-0 top-0 w-full">
            <ProjectCard {...project} />
          </div>
        ))}
      </div>

      {/* ── Mobile: Fan swipe cards ── */}
      <div className="w-full flex items-center justify-center min-[809px]:hidden">
        <SwipeCards projects={projects} />
      </div>
    </section>
  );
}